...
private:
  ...
  std::string_view::iterator cursor_{data_.begin()};
};
...
export struct label {
  ...
  explicit label(std::size_t id, std::string name, std::string description = {},
                 std::string color = {})
...
export struct project {
  ...
  explicit project(std::size_t id, std::string name,
                   std::string description = {}, std::string color = {},
                   bool active = true)
      : id{id}, name(std::move(name)), description(std::move(description)),
        color(std::move(color)), active(active) {}
...
export struct group {
  ...
  explicit group(std::size_t id, std::size_t project, std::string name,
                 std::string description = {}, std::string color = {},
                 bool active = true)
      : id{id}, project(project), name(std::move(name)),
        description(std::move(description)), color(std::move(color)),
        active(active) {}
...
export struct task {
  ...
  explicit task(std::size_t id, std::size_t project, std::size_t group,
      ...
      : id{id}, project(project), group(group), title(std::move(title)),
        description(std::move(description)), status(status),
        after(std::move(after)), labels(std::move(labels)),
...
std::optional<data::tparse_error> parse_record(data::tstate &state,
                                               v2::parser &parser,
                                               std::span<tfield> fields) {
  ...
  do {
    ...
    std::expected<v2::parser::tresult, data::tparse_error> line =
        parser.parse();

    // TODO PARSE ERROR should be same as v2::parser::terror
    if (!line)
      return line.error();
...
std::optional<data::tparse_error> parse_header(data::tstate &state,
                                               v2::parser &parser,
...
parse(std::string_view input) {
  ...
  v2::parser parser(input);
  while (true) {

    std::expected<v2::parser::tresult, tparse_error> line = parser.parse();
